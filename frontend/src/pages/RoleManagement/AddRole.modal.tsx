...
import { moduleName, entityModule } from "../../constants/data";
import Toastify from "toastify-js";
import "toastify-js/src/toastify.css";
import { addRole } from "../../helpers/roleManagementAPIs";
import { fetchallBE } from "../../helpers/businessEntityAPIs";
function AddRole(props: any) {
  ...
  const [roleDetails, setRoleDetails] = useState<any>({
    ...
    associatedBusinessEntity: "ABG",
  });

  //   const bodyData = roleDetails.permissions.splice(0, 1);
  const data = {
    roleName: roleDetails.roleName,
    permissions: roleDetails.permissions,
    createdBy: roleDetails.createdBy,
    associatedBusinessEntity: roleDetails.associatedBusinessEntity,
  };
  ...
  const handlecheckChange = (event: any, item: any, index: any) => {
    ...
    setRoleDetails((prevState: { permissions: any[] }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setEntityNames(fetchedData.data);
  ...
  const toastMessage = () => {
    ...
    if (response === 201) {
      ...
      color = "red";
      msg = "Role Already Exists for this Entity";
      showToastmsg(msg, color);
    }
  };
  useEffect(() => {
  ...
  //   console.log(entityNames[0].companyName);
  return (
    <>
      <Modal
        {...props}
              ...
              <Col>
                {isAbg !== "Yes" ? (
                  <Form.Control
                    ...
                    disabled
                    value={
                      entityNameValue ? entityNameValue + "_SUPER_ADMIN" : ""
                    }
                ...
                {isAbg === "Yes" ? (
                  <div className="table-responsive">
                    <Table className="mb-0" bordered>
                      ...
                      <tbody>
                        {(moduleName || []).map((item: any, index: number) => {
                          return (
                            <tr key={index}>
